 ...
 * Returns nmea_t (int).
 */
NMEA_API nmea_t nmea_get_type(const char *sentence);
 ...
 */
NMEA_API uint8_t nmea_get_checksum(const char *sentence);
 ...
 */
NMEA_API int nmea_has_checksum(const char *sentence, size_t length);
 ...
 */
NMEA_API int nmea_validate(const char *sentence, size_t length, int check_checksum);
 ...
 */
NMEA_API void nmea_free(nmea_s *data);
 ...
 */
NMEA_API nmea_s *nmea_parse(char *sentence, size_t length, int check_checksum);

/**
 * Explicitly initialize the NMEA library.
 * On platforms without __attribute__((constructor)), call this once before
 * using any other nmea functions.
 */
NMEA_API void nmea_init(void);

/**
 * Explicitly cleanup the NMEA library.
 * On platforms without __attribute__((destructor)), call this before exit.
 */
NMEA_API void nmea_cleanup(void);
