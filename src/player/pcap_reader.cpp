...
bool PcapReader::open(const std::string& pcap_path, const std::string& metadata_path) {
    ...
    try {
        ...
        auto packet_format = std::make_shared<osdk_core::PacketFormat>(info);
        ...
        while (pcap_file.good() && !pcap_file.eof()) {
            ...
            try {
                ...
                if (payload_size != packet_format->lidar_packet_size) {
                    continue;
                }
                ...
                spdlog::warn("PcapReader: Skipping malformed packet");
                continue;
            }
        }
